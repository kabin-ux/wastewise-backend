...
export const verifyJWT = (req, res, next) => {
  ...
  try {
    const decoded = jwt.verify(token, ACCESS_TOKEN_SECRET); // Use ACCESS_TOKEN_SECRET for access token verification
...
export const authorizeUserType = (...allowedUserTypes) => (req, res, next) => {
  ...
  if (!allowedUserTypes.includes(req.user?.userType)) {
    return res.status(403).json({ 
      StatusCode: 403,
      IsSuccess: false,
      ErrorMessage: 'Access forbidden: insufficient privileges' 
    });
  }
